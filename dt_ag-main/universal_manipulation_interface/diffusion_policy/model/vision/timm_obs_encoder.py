...
class TimmObsEncoder(ModuleAttrMixin):
    ...
    def aggregate_feature(self, feature):
        if self.model_name.startswith('vit'):
            assert self.feature_aggregation is None # vit uses the CLS token for policy inference.
            return feature[:, 0, :]
    ...
    def forward(self, obs_dict):
        ...
        # process rgb input
        # TODO: support different vision model encoders
        for key in self.rgb_keys:
            ...
            img = torch.clamp((img - img.min()) / (img.max() - img.min()), 0, 1)
