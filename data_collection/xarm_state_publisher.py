#!/usr/bin/env python3
"""
XArm State Publisher Node

Publishes the current state of the XArm robot:
- robot_obs/pose: Current TCP pose (PoseStamped)
- robot_obs/gripper: Current gripper position (Float32, normalized 0-1)
- robot_obs/joint_states: Current joint angles (JointState)

This node is read-only and does NOT send any commands to the robot.
"""

import time
import rclpy
from rclpy.node import Node
from geometry_msgs.msg import PoseStamped
from sensor_msgs.msg import JointState
from std_msgs.msg import Float32
from xarm.wrapper import XArmAPI
from scipy.spatial.transform import Rotation as R


class XArmStatePublisher(Node):
    """
    ROS2 node that publishes XArm state information.
    """
    def __init__(self):
        super().__init__('xarm_state_publisher')

        # Declare parameter for xarm IP address
        self.ip = self.declare_parameter('xarm_ip', '192.168.1.153').value  # Changed from .203
        self.get_logger().info(f'Connecting to xArm at IP: {self.ip}')

        # Initialize XArm for state reading only
        self.arm = XArmAPI(self.ip)
        self.setup_xarm()

        # Publishers with clear observation namespace
        self.pose_publisher = self.create_publisher(PoseStamped, 'robot_obs/pose', 10)
        self.gripper_state_pub = self.create_publisher(Float32, 'robot_obs/gripper', 10)
        self.joints_publisher = self.create_publisher(JointState, 'robot_obs/joint_states', 10)

        # Timer for state publishing at 30Hz
        self.state_timer = self.create_timer(1.0 / 30.0, self.state_timer_callback)

        self.get_logger().info('XArm State Publisher initialized successfully')

    def setup_xarm(self):
        """
        Initialize the XArm connection for state reading.
        """
        self.arm.motion_enable(enable=True)
        time.sleep(1)
        self.get_logger().info('XArm connection established for state publishing')

    def state_timer_callback(self):
        """
        Timer callback to publish current XArm state at 30Hz.
        """
        try:
            # Get current arm position
            code, pose = self.arm.get_position(is_radian=True)
            if code != 0:
                self.get_logger().warn(f'Failed to get XArm position: error code {code}')
                return

            # Convert position from mm to m
            pose[0:3] = [x / 1000.0 for x in pose[0:3]]

            # Publish current pose
            self.publish_pose(pose)

            # Get and publish joint angles
            code, angles = self.arm.get_joint_states(is_radian=True)
            if code == 0:
                self.publish_angles(angles[0] if angles else [])

            # Get and publish gripper state
            code, gripper_val = self.arm.get_gripper_position()
            if code == 0:
                # Normalize gripper value from [0-850] to [0-1] range
                # 850 = fully open (0), 0 = fully closed (1)
                normalized_gripper_val = (850.0 - gripper_val) / 850.0
                normalized_gripper_val = max(0.0, min(1.0, normalized_gripper_val))

                msg = Float32()
                msg.data = float(normalized_gripper_val)
                self.gripper_state_pub.publish(msg)

        except Exception as e:
            self.get_logger().error(f'Error in state callback: {e}')

    def publish_pose(self, pose):
        """
        Publish the current XArm pose as PoseStamped.
        Position in meters, orientation as quaternion.
        """
        x, y, z, roll, pitch, yaw = pose

        msg = PoseStamped()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.header.frame_id = "base_link"

        msg.pose.position.x = float(x)
        msg.pose.position.y = float(y)
        msg.pose.position.z = float(z)

        # Convert Euler (radians) to quaternion
        quaternion = R.from_euler('xyz', [roll, pitch, yaw]).as_quat()
        msg.pose.orientation.x = float(quaternion[0])
        msg.pose.orientation.y = float(quaternion[1])
        msg.pose.orientation.z = float(quaternion[2])
        msg.pose.orientation.w = float(quaternion[3])

        self.pose_publisher.publish(msg)

    def publish_angles(self, angles):
        """
        Publish the current XArm joint angles.
        """
        msg = JointState()
        msg.header.stamp = self.get_clock().now().to_msg()
        msg.name = [f'joint_{i+1}' for i in range(len(angles))]
        msg.position = [float(a) for a in angles]
        self.joints_publisher.publish(msg)

    def destroy_node(self):
        """
        Cleanly disconnect from XArm before shutting down.
        """
        self.get_logger().info('Disconnecting from XArm...')
        try:
            self.arm.disconnect()
        except:
            pass
        super().destroy_node()


def main(args=None):
    rclpy.init(args=args)

    node = XArmStatePublisher()

    try:
        node.get_logger().info('Running XArm State Publisher...')
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    finally:
        node.destroy_node()
        rclpy.shutdown()


if __name__ == '__main__':
    main()
